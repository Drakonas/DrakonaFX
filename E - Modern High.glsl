...
|| Drakonas (With Tino, ported ReShade's Gaussian AnamFlare & Bloom                 ||
|########################               By Tino               ######################||
\*==================================================================================*/
/*
[configuration]
...
StepAmount = 0.10
DefaultValue = 1.5
DependentOption = I_FILM_GRAIN_PASS
...
DependantOption = I_FILM_GRAIN_PASS
Input0=PreviousPass
Input0Filter=Linear
Input0Mode=Clamp
[/configuration]
...
float4 PS_AO_Blur(float2 axis)
{
	...
	for (int r = -6; r <= 6; ++r)
	{
		...
		weight *= max(0.0, 1.0 - (1000.0 * GetOption(B_AO_SHARPNESS)) * abs(tempdepth - centerdepth));
		sum += temp * weight;
		totalweight += weight;
	}
...
void Merger_SSAO()
{
	...
	{
		value = Sample();
		if (OptionEnabled(A_TEXTURE_SHARPEN)) { value = TexSharpenPass(value); }
	}
#if A_SSAO_ENABLED != 0
...
void TextureSharpen()
{
	float4 value = float4(Sample());
#if A_SSAO_ENABLED == 0 && A_SSGI_ENABLED == 0
	value = TexSharpenPass(value);
#endif
	SetOutput(value);
}
